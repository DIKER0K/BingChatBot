...
BOT_TOKEN = '5747127396:AAEXYrvUMht9LJqUzxoQA-Xg4Tv8wO98tRI'
ALLOWED_USER_IDS = '965614231'
COOKIE_PATH = os.getenv('COOKIE_PATH', './cookies.json')

bot = telebot.TeleBot(BOT_TOKEN)
gbot = Chatbot(cookiePath=COOKIE_PATH)
not_allow_info = 'Ты не авторизирован!!!'
...
async def bingChat(messageText):
    response_dict = await gbot.ask(prompt=messageText, conversation_style=ConversationStyle.precise)
    ...
    if 'text' in response_dict['item']['messages'][1]:
        ...
        response = "Что-то произошло. Пожалуйста перезапусти чат"
    ...
    if 'suggestedResponses' in response_dict['item']['messages'][1]:
        ...
        markup = quick_markup({
            'Нет возможных ответов': {'url': 'https://bing.com/chat'}
        }, row_width=1)
    ...
    if 'maxNumUserMessagesInConversation' in response_dict['item']['throttling'] and 'numUserMessagesInConversation' in response_dict['item']['throttling']:
        ...
        response = response+"\n----------\n"
        response = response+"Сообщений в беседе : %d / %d" % (
    ...
    if numUserMessagesInConversation >= maxNumUserMessagesInConversation:
        await gbot.reset()
        response = response+"\nПроизошла автоматическая очистка истории беседы"
